...
/* DB Structure 
--------------------------------------------------------------------------------------------------------------------------------------
ID (int, AI, primary key) | workplace (varchar(255)) | title (varchar(255)) | 
...
switch($method) {
    ...
    case 'GET':
        ...
        // Define the array that actually stores the job data
            $job_item=array(
                "id" => $id,
                "workplace" => $workplace,
                "title" => $title,
            ...
            // Push the job data into our array
            array_push($arr, $job_item);
        }
    ...
    $result = array("message" => "No job found by that ID.");
    } break;
    ...
    case 'DELETE':
        ...
        if(!isset($id)) {
            ...
            // Happy result first
            if ($jobs->deleteOne($id)) {
                http_response_code(200); // OK code
                $result = array("message" => "Job with ID " . $id . " has been deleted.");
            } 
            // Sad result
            else {
                http_response_code(503); // Server error code
                $result = array("message" => "No such job found. Please check ID again.");
            }
        }
    break;
    ...
    case 'POST': 
        ...
        if(
            ...
            $jobs->workplace = $data->workplace;
            $jobs->title = $data->title;
            $jobs->startyear = $data->startyear;
            $jobs->endyear = $data->endyear;
            $jobs->startmonth = $data->startmonth;
            $jobs->endmonth = $data->endmonth;

            // Attempt post
            if ($jobs->createJob()) {
                http_response_code(201); // Success!
                $result = array("message" => "Added job.");
            } else {
                http_response_code(503); // Server error code
                $result = array("message" => "Could not add job.");
            }
        } else {
            http_response_code(400); // Bad request code
            $result = array("message" => "Could not add job. Please check all fields are filled in.");
        }
    break;
    ...
    case 'PUT': 
        ...
        if(
            ...
            if ($jobs->updateJob()) {
                http_response_code(201); // Success!
                $result = array("message" => "Updated job.");
            } else {
                http_response_code(503); // Server error code
                $result = array("message" => "Could not add job.");
            }
            ...
            http_response_code(400); // Bad request code
            $result = array("message" => "Could not update job. Please check all fields are filled in.");
        }
break;
